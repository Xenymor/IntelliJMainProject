...
public class SaveButton implements Button {
    ...
    public SaveButton(Vector2 pos, Vector2 size, int number) {
        this.pos = pos;
        this.size = size;
        this.number = number;
    ...
    public void paint(Graphics g) {
        ...
        g.drawString("Save", (int)(pos.getX()+size.getX()/5), (int)(pos.getY()+size.getY()/4+size.getY()/2.5));
    }
    ...
    public void clicked() {
        ...
        Main.saveLevel(Main.level.getObjects(), Main.level.getPlayer(), number-1);
        new Thread(() -> {
            try {
