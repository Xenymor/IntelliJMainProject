    ...
    public static class MyPanel extends JPanel {
        ...
        public void paint(Graphics g) {
            ...
            if (lastMovedPlayer == PLAYER1) {
                ...
                System.out.println("Completed Calculation: " + duration);
        ...
        private static MoveWithScore getBotMove(PlayerType[][] fieldOriginal, PlayerType botType, int depth, int alpha, int beta, Move savedMove) {
            if (depth == 0) {
                return new MoveWithScore(savedMove, getScore(savedMove, fieldOriginal));
            }
            boolean PVfound = false;
            int best = MIN_VALUE;
            Move bestMove = null;
            List<Move> allMoves = getAllAllowedMoves(fieldOriginal, botType);
            for (Move move : allMoves) {
                int score;
                doMove(move, fieldOriginal);
                if (PVfound) {
                    MoveWithScore botMove = getBotMove(fieldOriginal, getEnemyType(botType), depth - 1, -alpha - 1, -alpha, move);
                    score = -botMove.getScore();
                    if (score > alpha && score < beta) {
                        MoveWithScore botMove1 = getBotMove(fieldOriginal, getEnemyType(botType), depth - 1, -beta, -score, move);
                        score = -botMove1.getScore();
                    }
                } else {
                    MoveWithScore botMove = getBotMove(fieldOriginal, getEnemyType(botType), depth - 1, -beta, -alpha, move);
                    score = -botMove.getScore();
                }
                undoMove(move, fieldOriginal, botType);
                if (score > best) {
                    if (score >= beta)
            ...
            return new MoveWithScore(bestMove, best);
        }

        private static void undoMove(Move move, PlayerType[][] field, PlayerType botType) {
            field[move.getxFrom()][move.getyFrom()] = botType;
            try {
                field[move.getxTo()][move.getyTo()] = NEUTRAL;
            } catch (ArrayIndexOutOfBoundsException ignored) {

            }
        }

        private static void doMove(Move move, PlayerType[][] field) {
            PlayerType type = field[move.getxFrom()][move.getyFrom()];
            field[move.getxFrom()][move.getyFrom()] = NEUTRAL;
            try {
                field[move.getxTo()][move.getyTo()] = type;
            } catch (ArrayIndexOutOfBoundsException ignored) {
        ...
        private static MoveWithScore getBotMove(PlayerType[][] fieldOriginal, PlayerType botType, int depth, PlayerType lastMovedPlayer) {
            PlayerType playerType = hasWon(lastMovedPlayer, fieldOriginal);
            if (playerType != NEUTRAL) {
                if (playerType == botType) {
                    return new MoveWithScore(new Move(-1, -1, -1, -1), Integer.MAX_VALUE);
                } else if (playerType == lastMovedPlayer) {
                    return new MoveWithScore(new Move(-1, -1, -1, -1), MIN_VALUE + 1);
                }
            }
            if (depth >= 7) System.out.println("Calculation Depth: " + depth);
            MoveWithScore bestMove = new MoveWithScore(new Move(-1, -1, -1, -1), MIN_VALUE);
            for (int x = 0; x < fieldSizeX; x++) {
                for (int y = 0; y < fieldSizeY; y++) {
        ...
        private static int getScore(PlayerType[][] field, PlayerType botType) {
            int score = 0;
            final PlayerType enemyType = getEnemyType(botType);
            for (int x = 0; x < fieldSizeX; x++) {
                for (int y = 0; y < fieldSizeY; y++) {
                    if (field[x][y] == botType) {
                        score--;
                    }
                    if (field[x][y] == enemyType) {
                        score++;
                    }
                }
            }
        ...
        public static List<Move> getAllAllowedMoves(PlayerType[][] field, PlayerType type) {
                    ...
                    if (field[x][y] == type) {
                        for (int x2 = -1; x2 <= 1; x2++) {
                            for (int y2 = -1; y2 <= 1; y2++) {
                                try {
                                    if (moveAllowed(field[x][y], x, y, x + x2, y + y2, field)) {
                                        allowedMoves.add(new Move(x, y, x + x2, y + y2));
                                    }
                                } catch (ArrayIndexOutOfBoundsException ignored) {
        ...
        static class MoveWithScore {
            ...
            public MoveWithScore(MoveWithScore moveWithScore) {
                this.move = moveWithScore.move;
                this.score = moveWithScore.score;
            }
